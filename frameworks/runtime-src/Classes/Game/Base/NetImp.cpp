...
#include "Battle/MessageDef.h"
#include "Battle/LogicFrame.h"
#include "Battle/UserData.h"
...
void NetImp::onRead(BitStream &packet_stream)
{
	...
	*/
	...
	switch (id)
	{
	case NET_SC_LOGIC_FRAME:
		{
			auto it = args.begin();
			++it;
			++it;
			int count = (*it).intValue();

			LogicFrame *frame = new LogicFrame;
			...
			for (int i=0; i<count; ++i)
			{
				...
				int uid = (*it).intValue();
				++it;
				LogicFrameAction::Type type = (LogicFrameAction::Type)(*it).intValue();
				++it;
			...
			LogicFrameManager::getInstance()->addFrame(frame);
		}
		break;
	case NET_SC_START:
		{
			...
			++it;
			int uid = (*it).intValue();
			UserData::getInstance()->setUid(uid);
		}
	default:
		break;
	}
